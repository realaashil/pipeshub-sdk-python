"""Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT."""

from __future__ import annotations
from .connectorconfig import ConnectorConfig, ConnectorConfigTypedDict
from .updateconnectorauthrequest import (
    UpdateConnectorAuthRequest,
    UpdateConnectorAuthRequestTypedDict,
)
from pipeshub.types import BaseModel, UNSET_SENTINEL
from pipeshub.utils import FieldMetadata, PathParamMetadata, RequestMetadata
import pydantic
from pydantic import model_serializer
from typing import Optional
from typing_extensions import Annotated, NotRequired, TypedDict


class UpdateConnectorAuthConfigRequestTypedDict(TypedDict):
    connector_id: str
    body: UpdateConnectorAuthRequestTypedDict
    r"""Request payload"""


class UpdateConnectorAuthConfigRequest(BaseModel):
    connector_id: Annotated[
        str,
        pydantic.Field(alias="connectorId"),
        FieldMetadata(path=PathParamMetadata(style="simple", explode=False)),
    ]

    body: Annotated[
        UpdateConnectorAuthRequest,
        FieldMetadata(request=RequestMetadata(media_type="application/json")),
    ]
    r"""Request payload"""


class UpdateConnectorAuthConfigResponseTypedDict(TypedDict):
    r"""Auth configuration updated"""

    success: NotRequired[bool]
    config: NotRequired[ConnectorConfigTypedDict]
    r"""Configuration for a connector instance including auth, sync, and filter settings"""


class UpdateConnectorAuthConfigResponse(BaseModel):
    r"""Auth configuration updated"""

    success: Optional[bool] = None

    config: Optional[ConnectorConfig] = None
    r"""Configuration for a connector instance including auth, sync, and filter settings"""

    @model_serializer(mode="wrap")
    def serialize_model(self, handler):
        optional_fields = set(["success", "config"])
        serialized = handler(self)
        m = {}

        for n, f in type(self).model_fields.items():
            k = f.alias or n
            val = serialized.get(k)

            if val != UNSET_SENTINEL:
                if val is not None or k not in optional_fields:
                    m[k] = val

        return m
