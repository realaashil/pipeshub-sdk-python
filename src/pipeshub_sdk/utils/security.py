...
def get_security_from_env(security: Any, security_class: Any) -> Optional[BaseModel]:
    ...
    if os.getenv("PIPESHUB_BEARER_AUTH"):
        security_dict["bearer_auth"] = os.getenv("PIPESHUB_BEARER_AUTH")

    if os.getenv("PIPESHUB_CLIENT_ID"):
        security_dict.setdefault("oauth2", {})["client_id"] = os.getenv(
            "PIPESHUB_CLIENT_ID"
        )

    if os.getenv("PIPESHUB_CLIENT_SECRET"):
        security_dict.setdefault("oauth2", {})["client_secret"] = os.getenv(
            "PIPESHUB_CLIENT_SECRET"
        )

    if os.getenv("PIPESHUB_TOKEN_URL"):
        security_dict.setdefault("oauth2", {})["token_url"] = os.getenv(
            "PIPESHUB_TOKEN_URL"
        )
